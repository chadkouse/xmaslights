...
class LightController {
        ...
        c.mousedown((e) => {
            ...
            for (let i = 0; i < this.program.getFrame(this.frameNumber).lightStrings.length; i++) {
                for (let bulb of this.program.getFrame(this.frameNumber).lightStrings[i].bulbs) {
                    let x = bulb.posX;
                    let y = bulb.posY;
                    let w = (this.bulbRadius + this.bulbLineWidth)*4;
                    let h = w;
    ...
    insertFrame() {
        ...
        for(let lightString of oldFrame.lightStrings) {
            ...
            const newLightString = new LightString(lightString.pin, lights);
        ...
        if (lightCount > 0 && lightCount <= 600) {
            for (var s = 0; s < 1; s++) {
                ...
                if (this.program.frames.length <= 0) {
                    ...
                    const lightString = new LightString(pin, lights);
                    ...
                    for (let f = 0; f < this.program.frames.length; f++) {
                        ...
                        let lightString = new LightString(pin, lights);
    ...
    scrollFrame() {
        ...
        for (let string of frame.lightStrings) {
            ...
            const newLightString = new LightString(string.pin, lights);
            lightStrings.push(newLightString);
        }
    ...
    loadProgram(json) {
        ...
        for (let frame of data.frames) {
            ...
            for (let string of frame.lightStrings) {
                ...
                const newLightString = new LightString(string.pin || 13, lights);
                lightStrings.push(newLightString);
            }
            const newFrame = new LightProgramFrame(lightStrings);
